...
class SubscriptionsApi {
  ...
  ///   This is a boolean that will decided if we are want to return the transferable data (default) or not(performance enhancement)
  ///
  /// * [SeededSubscription] seededSubscription:
  Future<Response> subscriptionsCreateNewSubscriptionWithHttpInfo({ bool? transferables, SeededSubscription? seededSubscription, }) async {
    // ignore: prefer_const_declarations
    final path = r'/subscriptions/create';

    // ignore: prefer_final_locals
    Object? postBody = seededSubscription;
  ...
  Future<Subscription?> subscriptionsCreateNewSubscription({ bool? transferables, SeededSubscription? seededSubscription, }) async {
    final response = await subscriptionsCreateNewSubscriptionWithHttpInfo( transferables: transferables, seededSubscription: seededSubscription, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<Response> subscriptionsDeleteSpecificSubscriptionWithHttpInfo(String subscription,) async {
    // ignore: prefer_const_declarations
    final path = r'/subscriptions/{subscription}/delete'
  ...
  /// * [String] subscription (required):
  ///   This is a identifier that is used to identify a specific subscription
  Future<void> subscriptionsDeleteSpecificSubscription(String subscription,) async {
    final response = await subscriptionsDeleteSpecificSubscriptionWithHttpInfo(subscription,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
