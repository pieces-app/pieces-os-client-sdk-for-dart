...
class AnchorPointsApi {
  ...
  Future<Response> anchorPointsCreateNewAnchorPointWithHttpInfo({ bool? transferables, SeededAnchorPoint? seededAnchorPoint, }) async {
    ...
    Object? postBody = seededAnchorPoint;
  ...
  Future<AnchorPoints?> anchorPointsSnapshot({ bool? transferables, }) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'AnchorPoints',) as AnchorPoints;
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> anchorPointsStreamIdentifiersWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/anchor_points/stream/identifiers';
  ...
  Future<StreamedIdentifiers?> anchorPointsStreamIdentifiers() async {
    final response = await anchorPointsStreamIdentifiersWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
