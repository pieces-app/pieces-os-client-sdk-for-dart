...
class UserApi {
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> userAccessTokenWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/user/access_token';
  ...
  Future<String?> userAccessToken() async {
    final response = await userAccessTokenWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> userPaddleRefreshWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/user/paddle/refresh';
  ...
  Future<RefreshedPaddleUser?> userPaddleRefresh() async {
    final response = await userPaddleRefreshWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'RefreshedPaddleUser',) as RefreshedPaddleUser;
