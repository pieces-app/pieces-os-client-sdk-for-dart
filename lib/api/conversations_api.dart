...
class ConversationsApi {
  ...
  Future<ConversationsCreateFromAssetOutput?> conversationsCreateFromAsset(String asset,) async {
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ConversationsCreateFromAssetOutput',) as ConversationsCreateFromAssetOutput;
  ...
  Future<Response> conversationsCreateFromWorkstreamSummaryWithHttpInfo(String workstreamSummary,) async {
    // ignore: prefer_const_declarations
    final path = r'/conversations/create/from_workstream_summary/{workstream_summary}'
  ...
  Future<ConversationsCreateFromWorkstreamSummaryOutput?> conversationsCreateFromWorkstreamSummary(String workstreamSummary,) async {
    final response = await conversationsCreateFromWorkstreamSummaryWithHttpInfo(workstreamSummary,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ConversationsCreateFromWorkstreamSummaryOutput',) as ConversationsCreateFromWorkstreamSummaryOutput;
  ...
  Future<Response> conversationsCreateSpecificConversationWithHttpInfo({ bool? transferables, SeededConversation? seededConversation, }) async {
    ...
    // ignore: prefer_final_locals
    Object? postBody = seededConversation;
  ...
  Future<FlattenedConversations?> conversationsIdentifiersSnapshot() async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'FlattenedConversations',) as FlattenedConversations;
  ...
  Future<SearchedConversations?> searchConversations({ bool? transferables, SearchInput? searchInput, }) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'SearchedConversations',) as SearchedConversations;
