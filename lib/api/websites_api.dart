...
class WebsitesApi {
  ...
  Future<SearchedWebsites?> searchWebsites({ bool? transferables, SearchInput? searchInput, }) async {
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'SearchedWebsites',) as SearchedWebsites;
  ...
  Future<Websites?> websitesSnapshot({ bool? transferables, }) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'Websites',) as Websites;
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> websitesStreamIdentifiersWithHttpInfo() async {
    // ignore: prefer_const_declarations
    final path = r'/websites/stream/identifiers';
  ...
  Future<StreamedIdentifiers?> websitesStreamIdentifiers() async {
    final response = await websitesStreamIdentifiersWithHttpInfo();
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
