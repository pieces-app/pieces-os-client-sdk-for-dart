...
class SubscriptionApi {
  ...
  Future<Response> subscriptionScoresIncrementWithHttpInfo(String subscription, { SeededScoreIncrement? seededScoreIncrement, }) async {
    // ignore: prefer_const_declarations
    final path = r'/subscription/{subscription}/scores/increment'
      .replaceAll('{subscription}', subscription);
  ...
  ///
  /// * [SeededScoreIncrement] seededScoreIncrement:
  Future<void> subscriptionScoresIncrement(String subscription, { SeededScoreIncrement? seededScoreIncrement, }) async {
    final response = await subscriptionScoresIncrementWithHttpInfo(subscription,  seededScoreIncrement: seededScoreIncrement, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
  ...
  Future<Response> subscriptionUpdateWithHttpInfo({ bool? transferables, Subscription? subscription, }) async {
    // ignore: prefer_const_declarations
  ...
  Future<Subscription?> subscriptionUpdate({ bool? transferables, Subscription? subscription, }) async {
    final response = await subscriptionUpdateWithHttpInfo( transferables: transferables, subscription: subscription, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
