...
import '/model/conversation_message_sentiment_enum.dart' show ConversationMessageSentimentEnum;
...
class ConversationMessage {
  ...
  @override
  String toString() => 'ConversationMessage[schema=$schema, id=$id, created=$created, updated=$updated, deleted=$deleted, model=$model, fragment=$fragment, conversation=$conversation, sentiment=$sentiment, role=$role, score=$score, annotations=$annotations]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ConversationMessage[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ConversationMessage[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, dynamic> mapToJson(Map<String, ConversationMessage> map) {
